  ...
  */
  loadRealm() {
    ...
    const conlangName = realm.nameConlang
      ? realm.nameConlang.map((word) =>
        this.gameState.conlang.getWord(word.toLowerCase())
      ).join(" ")
      : realm.name
  ...
  placeEntities(realm) {
    ...
    for (let y = 0; y < this.config.rows && y < realm.entities.length; y++) {
      for (
        let x = 0;
        x < this.config.cols && x < realm.entities[y].length;
        x++
      ) {
  ...
  updateCamera() {
    ...
    if (this.gameState.player.x < this.camera.x - marginX + 1) {
      this.camera.x = this.clamp(
        this.gameState.player.x + marginX - 1,
        marginX,
        this.config.cols - marginX - 1,
      )
    }
    if (this.gameState.player.x > this.camera.x + marginX - 1) {
      this.camera.x = this.clamp(
    ...
    if (this.gameState.player.y < this.camera.y - marginY + 1) {
      this.camera.y = this.clamp(
        this.gameState.player.y + marginY - 1,
        marginY,
        this.config.rows - marginY - 1,
      )
    }
    if (this.gameState.player.y > this.camera.y + marginY - 1) {
      this.camera.y = this.clamp(
    ...
    const offsetX = this.clamp(
      this.camera.x - marginX,
      0,
      this.config.cols - this.config.viewWidth,
    ) * this.tileWidth
    const offsetY = this.clamp(
      this.camera.y - marginY,
      0,
      this.config.rows - this.config.viewHeight,
    ) * this.tileHeight
  ...
  resizeView() {
    ...
    const worldEl = document.getElementById("world")
    worldEl.style.gridTemplateColumns =
      `repeat(${this.config.cols}, ${this.tileWidth}px)`
    worldEl.style.gridTemplateRows =
      `repeat(${this.config.rows}, ${this.tileHeight}px)`
