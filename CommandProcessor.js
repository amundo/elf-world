...
export class CommandProcessor {
  ...
  processCommand(input) {
    ...
    if (!this.gameState.conlang.hasWord(trimmedInput)) {
      this.gameState.ui.addMessage(
        this.gameState.ui.getUIText("unknownCommand"),
        "error",
      )
      return
    }
  ...
  handleWhatCommand() {
    if (this.gameState.focusedEntity) {
      ...
      this.gameState.ui.addMessage(
        `${learnedText}: ${this.gameState.focusedEntity.concept} = ${entityWord}`,
        "info",
      )
    } else {
      this.gameState.ui.addMessage(
        this.gameState.ui.getUIText("notNear"),
        "error",
      )
    }
  }
  ...
  handleHaveCommand() {
    ...
    const inventory = this.gameState.player
  ...
  handleSeeCommand() {
    if (this.gameState.focusedEntity) {
      const isKnown = this.gameState.knownWords.has(
        this.gameState.focusedEntity.concept,
      )
      const entityName = isKnown
        ? this.gameState.focusedEntity.conlangName
        : "???"
